...
def simulate(params = [0.5, 1, 0.01], dimension1 = 'A', subject = 0, rng = None, seed = 999):
    ...
    tmp_theta = np.array((0.01)).reshape(1,1)  # dimension weight
    alpha_d = np.array((0.01)).reshape(1,1)
    ...
    for stage in range(len(stages)):
        for stage_t in range(max_t):
            ...
            if stage in [0]:     
                ...
                alpha_d = (alpha_d * (1-lambd)) + lambd * (reward[t][S[t].index(choice[t])] - val[t][S[t].index(choice[t])] )
                tmp_theta -= alpha * alpha_d * dcost_theta
            ...
            if (stage_t >= (pass_criteria-1))  & (all(target_feature[stage] in x for x in choice[-pass_criteria:])):
                ...
                results = {'pe':pe, 'choice': choice, 'val': val, 'feature_weights': feature_weights, 'reward': reward, 'feedback': feedback, 'stim_hist': S, 'inputs': inputs, 'A_weights':A_w,\
                           'B_weights': B_w, 'theta': theta, 'alpha': alpha, 'beta': beta,'init_weights': init_weights,\
...
def llIED(params,args):
   ...
   """
    ...
    if not likelihood:
        ...
        # primacy parameters initalised to 0.01 instead of being free parameter
        alpha = 1/(1 + np.exp(-aeby[0]))
        beta = np.exp(aeby[1])
        theta_0 = 0.01 
        lambd = 1/(1 + np.exp(-aeby[2]))
        alpha_d_0 = 0.01
    ...
    for t in range(len(choice)):
        ...
        if len(S[t][0]) == 4:
            ...
            theta -= alpha * alpha_d * dcost_theta
