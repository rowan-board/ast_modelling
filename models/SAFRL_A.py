...
def simulate(params = [0.5, 1, 0.5, 0.5], dimension1 = 'A', subject = 0, rng = None, seed = 999):
    ...
    w_a, w_b, tmp_theta, alpha_d = weights_0[0].copy(), weights_0[1].copy(), np.array((0.01)).reshape(1,1), np.array((0.01)).reshape(1,1)
    ...
    for stage in range(len(stages)):
        for stage_t in range(max_t):
            ...
            if stage in [0]: #simple RL on feature weights
                ...
                # dimension learning with pearce hall updating 
                alpha_d = (alpha_d * (1-lambd)) + lambd * (reward[t][stim_hist[t].index(choice[t])] - val[t][stim_hist[t].index(choice[t])] )
                tmp_theta -= eps * alpha_d * dcost_theta
            ...
            if (stage_t >= (pass_criteria-1))  & (all(target_feature[stage] in x for x in choice[-pass_criteria:])):
                ...
                results = {'pe':pe, 'choice': choice, 'val': val, 'feature_weights': feature_weights, 'reward': reward, 'feedback': feedback, 'stim_hist': stim_hist, 'inputs': inputs, 'weights_a':weights_a,\
                           'weights_b': weights_b, 'theta': theta, 'alpha': alpha, 'beta': beta, 'eps': eps, 'weights_0': weights_0,\
...
def llIED(params,args):
   ...
   """
    ...
    if not likelihood:
        ...
        beta = np.exp(aeby[1])
        theta0 = 0.01
        eps = 1/(1 + np.exp(-aeby[2]))
        lambd = 1/(1 + np.exp(-aeby[3]))
        alpha_d_0 = 0.01
    ...
    for t in range(len(choice)):
        if len(S[t][0]) == 4:
            ...
            # dimension learning with pearce hall updating 
            alpha_d = (alpha_d * (1-lambd)) + lambd * (R[t][S[t].index(choice[t])] - V[t][S[t].index(choice[t])] )
            theta -= eps * alpha_d * dcost_theta
            alpha_t.append(alpha_d.copy())
            theta_t.append(theta.copy())

    if likelihood == True:
        return (np.prod(like), np.mean(like), like, -ll, theta_t, alpha_t)
